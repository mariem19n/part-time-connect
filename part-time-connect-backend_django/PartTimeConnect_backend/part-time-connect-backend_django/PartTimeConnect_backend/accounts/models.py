...
class UserRegistration(models.Model):
    ...
    def get_resumes(self):
        """Example implementation if not already defined"""
        if isinstance(self.resumes, list):
            return self.resumes
        try:
            return json.loads(self.resumes) if self.resumes else []
        except:
            return [self.resumes] if self.resumes else []
###############################
class UserProfile(models.Model):  # Stores additional user details
    ...
    resumes = models.TextField(blank=True, default="")
    ...
    def process_cv(self, pdf_file):
        ...
        try:
            ...
            resumes = json.loads(self.resumes) if self.resumes else []
            resumes.append({
                'path': pdf_file.name,
                'content': text
            })
            self.resumes = json.dumps(resumes)
            self.save()
